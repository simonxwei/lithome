...
/**
 * @see net.minecraft.world.level.biome.MultiNoiseBiomeSourceParameterList
 * @author simonxwei
 */
public final class MultiNoiseLithomeSourceParameterList {
    ...
    public MultiNoiseLithomeSourceParameterList(final Preset preset, final HolderGetter<Lithome> lithomes) {
        this.preset = preset;
    ...
    static {
        DIRECT_CODEC = RecordCodecBuilder.create(i -> i.group(
                Preset.CODEC.fieldOf("preset").forGetter(l -> l.preset),
                RegistryOps.retrieveGetter(LithomeRegistries.LITHOME)
        ).apply(i, MultiNoiseLithomeSourceParameterList::new));
        CODEC = RegistryFileCodec.create(LithomeRegistries.MULTI_NOISE_LITHOME_SOURCE_PARAMETER_LIST, DIRECT_CODEC);
    }

    public record Preset(Identifier id, SourceProvider provider) {

        public static final Preset OVERWORLD;
        private static final Map<Identifier, Preset> BY_NAME;
        public static final Codec<Preset> CODEC;

        static {
            OVERWORLD = new Preset(Constants.id("overworld"), OverworldLithomeBuilder::createParameters);
            BY_NAME = Stream.of(OVERWORLD).collect(Collectors.toMap(Preset::id, p -> p));
            CODEC = Identifier.CODEC.flatXmap(
                    i -> Optional.ofNullable(BY_NAME.get(i)).map(DataResult::success).orElseGet(() -> DataResult.error(() -> "Unknown Lithome parameter preset: " + i)),
        ...
        private interface SourceProvider {
            ...
            <T> LithomeClimate.ParameterList<T> apply(Function<ResourceKey<Lithome>, T> lookup);
        }
    }
}
